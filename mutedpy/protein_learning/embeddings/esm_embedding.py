...
class ESMEmbedding(Embedding):
    ...
    def __init__(self, name = 'esm-2', device = 'cpu', preloaded = None, save_location = None, mean = False, length = 91, huggingface_id = " "):
        self.name = name
        if name == 'esm2_t33_650M_UR50D':
            ...
            login(huggingface_id)
            client = ESMC.from_pretrained("esmc_600m", device=torch.device(device))  # or "cpu"
            def query(seq):
        ...
        elif name == 'esm3_sm_open_v1':
            login(huggingface_id)
            client = ESM3.from_pretrained("esm3_sm_open_v1", device=torch.device(device))  # or "cpu"
            def query(seq):
                protein = ESMProtein(sequence=(seq))
                protein_tensor = client.encode(protein)
    ...
    def embed_seq(self,x, device = None, batch_size = 10, verbose = False):
        ...
        if self.name == 'esm3_sm_open_v1' or self.name == 'esmc_600m':
