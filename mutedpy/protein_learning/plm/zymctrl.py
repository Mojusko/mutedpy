...
from mutedpy.protein_learning.plm.basemodel import BaseModel
from mutedpy.protein_learning.plm.utils import SpecialTokens
from  mutedpy.protein_learning.plm.model_utils import GenerateSeqCallback
...
class ZymCTRL(BaseModel):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        try:
            ...
            raise LookupError(f"Model not found at {self.model_dir}")
    ...
    def run_training(self,
        ...
        try:
            print ("Instantiating model from json")
            config = GPT2Config.from_json_file(self.model_dir+'/config.json')
        except FileNotFoundError:
            print ("Instantiating model from json as pretrained")
        ...
        trainer.save_model(output_dir = cfg.training.save_dir)
    ...
    def embed(self,x):
        ...
        Parameters
        ----------
        x: list of strings of input sequence
        ...
        embeddings = []
        for s in x:         
            input = self.tokenizer(s, return_tensors='pt').to(self.device)
            embedding = self.model(input.input_ids, output_hidden_states=True).hidden_states[-1]
    ...
    def generate(self,
        ...
        with open(cfg.generation.output_file, "w") as file:
            pass
